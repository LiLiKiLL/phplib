 ...
 */
function getDateListOfPeriod($startTime, $endTime) {
 ...
 * @return boolean
 */
function jsonParamValidate($param, $requiredKeys = array(), $level = 1) {
    $result = true;
 ...
 */
function arrayRebuild($data, $keyKeyName, $valueKeyName) {
    $result = array();
    foreach ($data as $k => $v) {
 ...
 */
function phoneMosaic($phone, $replace = '****') {
 ...
 */
function contentBreif($content, $replace = '...', $length = 140, $encoding = 'utf-8') {
 ...
 */
function enSentenceReverse($sentence, $delimiter = ' ') {
 ...
 * @return string           [description]
 */
function sentenceReverse($sentence, $encoding = 'utf-8') {
 ...
 */
function tabToSpace($content, $tabSpace = 4) {
    $pattern = '/[\t]/';
...
function age($birthDate) {
    ...
    $now = time();
    $date = date('Y-m-d', $now);
    list($ny, $nm, $nd) = explode('-', $date);
    list($by, $bm, $bd) = explode('-', $birthDate);
    $birthTime = strtotime($birthDate);

    $birth['second'] = $now - $birthTime;// 出生到现在秒数 = 现在时间戳 - 生日时间戳
    $birth['minute'] = ceil($second / 60);// 出生到现在分钟数
    $birth['hour'] = ceil($second / 3600);// 出生到现在小时数
    $birth['day'] = ceil($second / 86400);// 出生到现在天数
    $birth['age'] = $ny - $by;// 岁数 = 现在年份 - 出生年份
    // 具体到年月日时分秒
 ...
 */
function getDesc(&$data, $originKey, $descMap) {
 ...
 * @return [type]             [description]
 */
function timestampsFormat(&$data, $keys = ['create_at', 'update_at', 'start_time', 'end_time'], $format = 'Y-m-d H:i:s') {
    foreach ($keys as $key) {
        if (isset($data[$key])) {
...
/**
 * 给数组赋默认值，只返回默认数组中存在的键，原数组其余键将被删除
 * @param  [type] $array   原始数组
 * @param  [type] $default 默认数组
 * @return [type]          [description]
 */
function assignArrayDefault($array, $default) {
    $result = array();
    foreach ($default as $k => $v) {
        $result[$k] = isset($array[$k]) ? $array[$k] : $v;
    }
